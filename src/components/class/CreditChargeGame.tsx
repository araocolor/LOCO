...
interface CreditChargeGameProps {
  onSuccess: () => void;
  onCancel: () => void;
}
...
const BRICK_COLORS = [
  "#ff7a59", "#ffb347", "#ffe066", "#77dd77",
  "#66d9e8", "#74c0fc", "#a78bfa", "#f783ac",
];
const BRICK_GAPS = new Set([
  "1-0", "1-9", "2-4", "2-5", "3-1", "3-8", "4-0", "4-9",
]);

const CACHE_KEY = "chat_friends_cache";

function loadFriendsFromCache(): GameMember[] {
  try {
    const raw = localStorage.getItem(CACHE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    const list = parsed.following ?? parsed.followers ?? [];
    const shuffled = [...list].sort(() => Math.random() - 0.5);
    return shuffled.slice(0, 3).map((f: { id: string; nickname: string; profile_image_url: string | null }) => ({
      userId: f.id,
      nickname: f.nickname,
...
function buildMemberLayouts(bounds: GameBounds, members: GameMember[]) {
...
function getRescueSlot(bounds: GameBounds, rescuedIndex: number) {
  ...
  return { x: startX + col * (chipSize + gap), y: startY, size: chipSize };
}
...
export default function CreditChargeGame({ onSuccess, onCancel }: CreditChargeGameProps) {
  ...
  const totalBreakableBricks = useMemo(
    () => buildBricks(bounds).filter((b) => b.alive).length,
    [bounds]
  );
  const memberCeiling = useMemo(() => {
    if (memberLayouts.length === 0) return 56;
    return Math.max(0, Math.min(...memberLayouts.map((l) => l.y)) - BALL_RADIUS - 4);
  }, [memberLayouts]);
  ...
  const remainingSeconds = Math.max(0, TIME_LIMIT - elapsedWholeSeconds);
  const isTimeWarning = remainingSeconds <= 10 && game?.launched;
  ...
  useEffect(() => {
    ...
    const updateBounds = () => {
      ...
      setBounds((c) => (c.width === next.width && c.height === next.height ? c : next));
    };
    updateBounds();
  ...
  useEffect(() => {
    ...
    const step = (time: number) => {
      ...
      setGame((current) => {
        ...
        if (current.startedAt && (time - current.startedAt) / 1000 >= TIME_LIMIT) {
          return {
            ...current,
            balls: [],
            time,
        ...
        const liveBrickCount = current.bricks.filter((b) => b.alive).length;
        ...
        const readyRescues = Object.entries(nextPendingReleaseAt).filter(([, at]) => at <= time);
        if (readyRescues.length > 0) {
          ...
          readyRescues.forEach(([memberId, releaseAt]) => {
            ...
            if (!nextRescuedOrder.includes(memberId)) nextRescuedOrder.push(memberId);
          });
        }
        ...
        for (let i = 0; i < current.balls.length; i++) {
          if (current.balls[i].vx === 0 && current.balls[i].vy === 0) paddleBallIndices.push(i);
        }
        ...
        for (let i = 0; i < current.balls.length; i++) {
          ...
          if (isOnPaddle) {
            const order = paddleBallIndices.indexOf(i);
            const total = paddleBallIndices.length;
            const spacing = BALL_RADIUS * 3;
            const offsetX = (order - (total - 1) / 2) * spacing;
          ...
          const mag = Math.hypot(nextBall.vx, nextBall.vy) || 1;
          nextBall.vx = (nextBall.vx / mag) * speedTarget;
          nextBall.vy = (nextBall.vy / mag) * speedTarget;
          ...
          const hitBrickIndex = nextBricks.findIndex(
            (b) =>
              b.alive &&
              nextBall.x + nextBall.radius >= b.x &&
              nextBall.x - nextBall.radius <= b.x + b.width &&
              nextBall.y + nextBall.radius >= b.y &&
              nextBall.y - nextBall.radius <= b.y + b.height
          );
          ...
          if (hitBrickIndex >= 0) {
            ...
            if (hitFromTop || hitFromBottom) nextBall.vy *= -1;
            else nextBall.vx *= -1;
          }

          const hitMember = memberLayouts.find((l) => {
            if (nextReleasedAt[l.member.userId] || nextPendingReleaseAt[l.member.userId]) return false;
            return (
              nextBall.x + nextBall.radius >= l.x &&
              nextBall.x - nextBall.radius <= l.x + l.width &&
              nextBall.y + nextBall.radius >= l.y &&
              nextBall.y - nextBall.radius <= l.y + l.height
            );
          });
        ...
        const remainingBricks = nextBricks.filter((b) => b.alive).length;
        const rescuedAll = members.length > 0 && nextRescuedOrder.length >= members.length;
        if (remainingBricks === 0 || rescuedAll) {
          return {
            ...current,
            ...
            launched,
            status: "completed",
            completionReason: rescuedAll ? "rescue" : "bricks",
            remainingLives,
            waitingOnPaddle,
          };
        }
  ...
  const isLastAttemptFailed = game?.status === "gameover" && remainingAttempts <= 0;
  ...
  function addBallFromReserve() {
    ...
    setGame((current) => {
      ...
      return {
        ...current,
        balls: [...current.balls, createBallOnPaddle(current.paddleX, bounds)],
  ...
  function movePaddle(clientX: number) {
    ...
    setGame((c) => (c ? { ...c, paddleX: nextX } : c));
  }

  return (
    <div className="fixed inset-0 z-[300] flex justify-center">
      <div className="relative w-full max-w-[500px] overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.48),_transparent_34%),linear-gradient(180deg,_#1f3357_0%,_#284976_48%,_#1b2640_100%)]">
        ...
        <div className="relative z-10 flex items-center justify-between px-5 pb-2 pt-5 text-white">
          <div>
            <p className="text-[11px] font-black tracking-[0.32em] text-white/70">외상충전 미션</p>
            <h3 className="mt-1 text-[22px] font-black tracking-[-0.03em]">회원을 구출하자</h3>
            <p className="mt-0.5 text-[12px] font-semibold text-white/50">40초 이내 클리어 시 10회 충전</p>
          </div>
          <div
            className={`rounded-full border px-3 py-1.5 text-right backdrop-blur ${
              isTimeWarning
                ? "border-red-400/30 bg-red-500/20"
                : "border-white/15 bg-white/10"
            }`}
          >
            <p className="text-[11px] font-bold text-white/60">남은시간</p>
            <p className={`text-sm font-bold ${isTimeWarning ? "text-red-300" : "text-white"}`}>
              {remainingSeconds}초
            </p>
          </div>
        </div>
        ...
        <div
          ref={boardRef}
          className="relative h-[calc(100vh-148px)] min-h-[calc(100vh-148px)] touch-none overflow-hidden"
        ...
        >
          ...
          {memberLayouts.map((layout) => {
            ...
            const shouldRenderFalling = Boolean(releasedAt && rescueSlot && fallProgress < 1);
            ...
            return (
              <div key={layout.member.userId}>
                ...
                {shouldRenderFalling && rescueSlot && (
                  <div
          ...
          {game && (
            <button
              ...
              className={`absolute z-20 -translate-x-1/2 select-none border border-yellow-100 bg-yellow-300 px-5 shadow-[0_12px_24px_rgba(250,204,21,0.35)] will-change-transform ${
                game.launched ? "rounded-full" : "flex items-center justify-center rounded-full"
              }`}
              style={{
                left: game.paddleX,
            ...
            <div className="absolute bottom-5 right-5 z-20 flex items-center gap-3">
              {Array.from({ length: displayReserve }, (_, i) => (
                <button
                  ...
                  className="block cursor-pointer rounded-full bg-[#fff4a8] shadow-[0_0_12px_rgba(255,244,168,0.8)] transition hover:scale-125"
          ...
          {game?.status === "gameover" && (
            <div className="absolute inset-0 z-40 flex items-center justify-center bg-[#0b1220]/68 backdrop-blur-[3px]">
              <div className="mx-6 w-full max-w-[320px] rounded-[28px] border border-white/12 bg-[#0f172a]/72 px-6 py-7 text-center text-white shadow-[0_18px_60px_rgba(15,23,42,0.35)]">
                <p className="text-[11px] font-black uppercase tracking-[0.34em] text-red-300/90">
                  미션 실패 {attemptCount}/{MAX_ATTEMPTS}
                </p>
                <h4 className="mt-3 text-[20px] font-black tracking-[-0.04em]">
                  {elapsedWholeSeconds >= TIME_LIMIT ? "시간 초과!" : "공을 모두 잃었어요"}
                </h4>
                <p className="mt-2 text-sm font-semibold text-white/78">
                  {isLastAttemptFailed
                    ? "기회를 모두 사용했습니다"
                    : `남은 기회 ${remainingAttempts}번 · 40초 이내 클리어 시 충전`}
                </p>
          ...
          {game?.status === "completed" && (
            <div className="absolute inset-0 z-40 flex items-center justify-center bg-[#0b1220]/68 backdrop-blur-[3px]">
              ...
              <div className="mx-6 w-full max-w-[320px] rounded-[28px] border border-white/12 bg-[#0f172a]/72 px-6 py-7 text-center text-white shadow-[0_18px_60px_rgba(15,23,42,0.35)]">
                <p className="text-[11px] font-black uppercase tracking-[0.34em] text-yellow-200/90">미션 성공!</p>
        ...
        <style jsx>{`
          ...
          @keyframes confetti-burst {
            0% { opacity: 0; transform: translate(-50%, -50%) rotate(0deg) scale(0.8); }
            12% { opacity: 1; }
            100% { opacity: 0; transform: translate(calc(-50% + var(--confetti-x)), calc(-50% + var(--confetti-y))) rotate(var(--confetti-rotate)) scale(1.05); }
          }
        `}</style>
      </div>
    </div>
