...
export async function GET(req: NextRequest) {
  ...
  const currentBalance = profile?.star_balance ?? 0;
  await admin
    .from("profiles")
    .update({ star_balance: currentBalance + payment.credit_amount })
    .eq("id", payment.user_id);

  return NextResponse.redirect(new URL("/?payment=success&tab=mypage", req.url));
}
