...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!friendship || !["approved", "friend"].includes(friendship.status)) {
      return NextResponse.json({ error: "친구 상태에서만 숨김할 수 있습니다." }, { status: 400 });
    }
    ...
    const { error } = await supabase.from("friend_member_states").upsert(
      {
        ...
        target_id,
        state: "hidden",
        previous_status: (currentState?.previous_status ?? friendship.status) as "approved" | "friend",
      },
