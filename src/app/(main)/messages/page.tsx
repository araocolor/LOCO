...
export default function MessagesPage() {
  ...
  async function handlePhotoUpload(file: File) {
    ...
    try {
      ...
      const path300 = `${currentUserId}/${ts}_300.webp`;
      const path1024 = `${currentUserId}/${ts}_1024.webp`;
      ...
      // content에 JSON으로 두 URL 저장
      const content = JSON.stringify({ type: "image", thumb: url300, full: url1024 });

      const { data: message, error } = await supabase
        .from("messages")
        .insert({ sender_id: currentUserId, receiver_id: selectedUserId, content })
        .select()
        .single();
  ...
  async function prefetchMessages(convList: Conversation[], myId: string) {
    const supabase = createClient();
    for (const conv of convList) {
      if (!conv.other_user) continue;
      ...
      if (msgs) {
        const limited = limitImageMessages(msgs);
        sessionStorage.setItem(cacheKey, JSON.stringify(limited));
        warmImageMessages(limited);
      }
    }
  }
  ...
  async function fetchConversations() {
    ...
    try {
      ...
      if (json.data) {
        ...
        if (user) prefetchMessages(json.data, user.id);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    async function loadCurrentUser() {
      ...
      if (user) {
        ...
        if (cached) prefetchMessages(JSON.parse(cached), user.id);
      }
    }
...
{/* 대화창 슬라이드 오버레이 */}
      ...
      >
        {/* 대화창 헤더 */}
        <div className="h-[70px] flex items-center gap-3 px-4 border-b border-gray-100">
          ...
          <span className="font-semibold text-gray-900 flex-1">{otherUser?.nickname ?? "로딩중..."}</span>
            ...
            messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-xs rounded-lg text-sm overflow-hidden ${
                      imageData ? "" : "px-3 py-2"
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-500 mt-1 block px-1">{formatTime(msg.sent_at)}</span>
                  </div>
                </div>
              );
            })
